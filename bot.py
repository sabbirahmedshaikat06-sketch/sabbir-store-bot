...
PRODUCTS = {
    ...
    "br_cs": {
        ...
        "short_name": "BR CS + TOURNAMENT LOCATION",
        "price": 250,
        "type": "file",
        "file_key": "br_cs",
    },
    "pest": {
        ...
        "short_name": "PEST TOURNAMENT LOCATION 🩵",
        "price": 180,
        "type": "file",
        "file_key": "pest",
    },
    "pink": {
        ...
        "short_name": "PINK TOURNAMENT LOCATION 💜",
        "price": 180,
        "type": "file",
        "file_key": "pink",
    },
    "yellow": {
        ...
        "short_name": "YELLOW TOURNAMENT LOCATION 💛",
        "price": 180,
        "type": "file",
        "file_key": "yellow",
    },
    "blue": {
        ...
        "short_name": "BLUE TOURNAMENT LOCATION 💙",
        "price": 180,
        "type": "file",
        "file_key": "blue",
    },
    "green": {
        ...
        "short_name": "GREEN TOURNAMENT LOCATION 💚",
        "price": 180,
        "type": "file",
        "file_key": "green",
    },
...
def ensure_user(tg_user, referral_by=None):
    ...
    if not exists:
        cur.execute("""
        INSERT INTO users
        (user_id, name, username, balance, referral_by,
         referral_rewarded, verified, created_at)
        VALUES (?, ?, ?, 0, ?, 0, 0, ?)
        """, (
            tg_user.id,
            tg_user.full_name,
...
def product_in_stock(product):
    ...
    cur.execute(
        "SELECT telegram_file_id FROM files WHERE file_key=?",
        (product["file_key"],)
    )
    row = cur.fetchone()
    con.close()
    return bool(row), 1 if row else 0


# =========================================================
# PRICE LIST / SHOP DESIGN
# =========================================================
...
🟢 BR CS + TOURNAMENT LOCATION → ৳250
🟢 PEST TOURNAMENT LOCATION 🩵 → ৳180
🟢 PINK TOURNAMENT LOCATION 💜 → ৳180
🟢 YELLOW TOURNAMENT LOCATION 💛 → ৳180
🟢 BLUE TOURNAMENT LOCATION 💙 → ৳180
🟢 GREEN TOURNAMENT LOCATION 💚 → ৳180
...
product order নেওয়া যাবে।
কোনো product-এর actual stock না থাকলে
payment verify হওয়ার পর টাকা refund করা হবে।

━━━━━━━━━━━━━━━━

👇 নিচে আপনার পছন্দের product select করুন:
"""
...
def shop_keyboard():
    ...
    for key, product in PRODUCTS.items():
        in_stock, count = product_in_stock(product)

        # Button-এ stock indicator থাকবে, কিন্তু price list-এ stock count দেখানো হবে না.
        icon = "✅" if in_stock else "❌"

        keyboard.append([
            InlineKeyboardButton(
                f"🛒 {icon} {product['short_name']} — ৳{product['price']}",
...
async def product_select(update, context):
    ...
    stock_status = "✅ IN STOCK AVAILABLE" if in_stock else "❌ CURRENT STOCK UNAVAILABLE"
...
⚠️ যদি actual stock available না থাকে,
payment verify হওয়ার পরে stock না পাওয়া গেলে
আপনার payment refund করা হবে।

📌 Select payment method below.
"""

    keyboard = [
        [InlineKeyboardButton("🇧🇩 bKash", callback_data="paymethod:bkash")],
        [InlineKeyboardButton("🟢 Nagad", callback_data="paymethod:nagad")],
        [InlineKeyboardButton("🔙 Back to Price List", callback_data="shop")]
    ]
...
""",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🧾 Transaction ID পাঠান", callback_data="send_tx")],
            [InlineKeyboardButton("🔙 Back", callback_data=f"product:{product_key}")],
...
""",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🇧🇩 bKash", callback_data="balancepay:bkash")],
            [InlineKeyboardButton("🟢 Nagad", callback_data="balancepay:nagad")],
            [InlineKeyboardButton("🔙 Back", callback_data="add_balance")]
        ])
    )
...
async def balance_transaction(update, context):
    ...
    VALUES (?, ?, ?, ?, ?, ?)
    """, (user.id, amount, method, tx_text, "pending", now_str()))
...
""",
        reply_markup=InlineKeyboardMarkup([
            [
                InlineKeyboardButton("✅ ACCEPT", callback_data=f"accept_balance:{tx_id}"),
                InlineKeyboardButton("❌ REJECT", callback_data=f"reject_balance:{tx_id}")
            ]
        ])
    )
...
async def text_handler(update, context):
    ...
    if context.user_data.get("waiting_tx"):
        ...
        cur.execute("""
        INSERT INTO orders
        (user_id, product_key, product_name, amount,
...
""",
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ ACCEPT", callback_data=f"accept_order:{order_id}"),
                    InlineKeyboardButton("❌ REJECT", callback_data=f"reject_order:{order_id}")
                ]
            ])
        )
...
async def profile(update, context):
    ...
    cur.execute("SELECT COUNT(*) FROM users WHERE referral_by=?", (user_id,))
...
""",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("💰 View Balance", callback_data="profile")],
            [InlineKeyboardButton("🔙 Back", callback_data="home")]
        ])
    )
...
async def lucky_spin(update, context):
    ...
    cur = con.cursor()
    cur.execute(
        "SELECT result FROM spins WHERE user_id=? AND spin_date=?",
...
""",
    ...
    await query.edit_message_text("🎰 — LUCKY SPIN —\n\n🎡 Spinning...\n\n⏳ Please wait...")
    ...
    cur = con.cursor()
    cur.execute(
        "INSERT INTO spins (user_id, spin_date, result) VALUES (?, ?, ?)",
    ...
    if result == 0:
        result_text = "😅 No Prize This Time!\n\nTomorrow আবার চেষ্টা করুন।"
    else:
...
async def download_file(update, context):
    ...
    if not purchased:
        await query.message.reply_text("❌ এই product আপনি purchase করেননি।")
        return

    if not file_row:
        await query.message.reply_text("❌ File এখন available নেই।")
        return
...
async def order_action(update, context):
    ...
    if product["type"] == "credential":
        ...
        if not credential:
            # No stock: automatically refund the order amount.
            cur.execute(
                "UPDATE orders SET status='refunded', delivered_at=? WHERE id=?",
                (now_str(), order_id)
            )
            con.commit()
            con.close()
            ...
            await query.edit_message_text(
                f"⚠️ Order #{order_id}\n\n❌ Stock unavailable.\n💰 Refund added to user's balance."
            )
...
✅ আপনার balance-এ refund যোগ করা হয়েছে।
...
"""
        ...
        cur.execute("""
        UPDATE credentials
        SET sold=1,sold_to=?,sold_at=?,expires_at=?
        WHERE id=?
        """, (
            user_id,
            now_str(),
        ...
        cur.execute("""
        UPDATE orders
        SET status='accepted',delivered_at=?
        WHERE id=?
        """, (now_str(), order_id))
...
"""
    ...
    if not file_row:
        # No file stock: automatically refund.
        cur.execute(
            "UPDATE orders SET status='refunded', delivered_at=? WHERE id=?",
            (now_str(), order_id)
        )
...
async def balance_action(update, context):
    ...
    cur.execute(
        "SELECT user_id,amount,status FROM transactions WHERE id=?",
        (txid,)
    )
    row = cur.fetchone()
    ...
    if action == "reject_balance":
        ...
        await query.edit_message_text(f"❌ Balance #{txid} rejected.")
        await context.bot.send_message(
            chat_id=user_id,
            text=f"❌ BALANCE REQUEST REJECTED\n\n💰 Amount: ৳{amount}\n\n📩 Support: {SUPPORT}"
        )
        return
    ...
    await query.edit_message_text(f"✅ Balance #{txid} accepted.")
...
async def admin(update, context):
    ...
    keyboard = [
        [InlineKeyboardButton("🔑 ADD USER/PASS - 7 DAY", callback_data="admin:add_credential:7")],
        [InlineKeyboardButton("🔑 ADD USER/PASS - 15 DAY", callback_data="admin:add_credential:15")],
        [InlineKeyboardButton("🔑 ADD USER/PASS - 30 DAY", callback_data="admin:add_credential:30")],
        [InlineKeyboardButton("📁 ADD BR CS FILE", callback_data="admin:file:br_cs")],
        [InlineKeyboardButton("📁 ADD PEST FILE 🩵", callback_data="admin:file:pest")],
        [InlineKeyboardButton("📁 ADD PINK FILE 💜", callback_data="admin:file:pink")],
        [InlineKeyboardButton("📁 ADD YELLOW FILE 💛", callback_data="admin:file:yellow")],
        [InlineKeyboardButton("📁 ADD BLUE FILE 💙", callback_data="admin:file:blue")],
        [InlineKeyboardButton("📁 ADD GREEN FILE 💚", callback_data="admin:file:green")],
        [InlineKeyboardButton("📊 ADMIN STATS", callback_data="admin:stats")]
    ]
...
"""
    ...
    if data == "admin:stats":
        ...
        cur.execute("SELECT COUNT(*) FROM credentials WHERE sold=0")
...
"""
        ...
        cur.execute("""
        INSERT INTO credentials (username,password,duration)
        VALUES (?,?,?)
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(
        verify_account, pattern="^verify_account$"
    ))
    ...
    app.add_handler(CallbackQueryHandler(lucky_spin, pattern="^spin$"))
    app.add_handler(CallbackQueryHandler(downloads, pattern="^downloads$"))
    ...
    # Add balance
    app.add_handler(CallbackQueryHandler(
        add_balance_menu, pattern="^add_balance$"
    ))
    app.add_handler(CallbackQueryHandler(
        balance_fixed_amount, pattern="^bal_amount:"
    ))
    app.add_handler(CallbackQueryHandler(
        custom_balance, pattern="^bal_custom$"
    ))
    app.add_handler(CallbackQueryHandler(
        balance_payment, pattern="^balancepay:"
    ))

    # Product
    app.add_handler(CallbackQueryHandler(
        product_select, pattern="^product:"
    ))
    app.add_handler(CallbackQueryHandler(
        payment_method, pattern="^paymethod:"
    ))
    app.add_handler(CallbackQueryHandler(
        send_tx, pattern="^send_tx$"
    ))

    # Download
    app.add_handler(CallbackQueryHandler(
        download_file, pattern="^download:"
    ))
    ...
    # Admin text
    app.add_handler(MessageHandler(
        filters.TEXT & filters.User(user_id=ADMIN_ID) & ~filters.COMMAND,
